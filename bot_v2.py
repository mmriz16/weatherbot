...
from report_state import build_report, render_status_text, sync_state_summary
...
MAX_BET          = _cfg.get("max_bet", 20.0)        # max bet per trade (absolute)
...
SCAN_INTERVAL    = _cfg.get("scan_interval", 600)    # every 10 min (was 3600)
CALIBRATION_MIN  = _cfg.get("calibration_min", 30)
VC_KEY           = _cfg.get("vc_key", "")
STOP_LOSS_PCT    = _cfg.get("stop_loss_pct", 0.30)  # price drop 30% from entry = cut
TAKE_PROFIT_MAX  = _cfg.get("take_profit_max", 0.85) # sell at $0.85 for sure profit
...
RUN_LOCK_FILE    = DATA_DIR / "weatherbot.lock"
...
def ensemble_probability(ecmwf_temp, hrrr_temp, metar_temp, t_low, t_high, sigma=2.0, weights=None):
    """
    Calculate probability using weighted ensemble of multiple forecast sources.
    
    Args:
        ecmwf_temp: ECMWF forecast temperature
        hrrr_temp: HRRR forecast temperature (None if not available)
        metar_temp: METAR current temperature (None if not available)
        t_low, t_high: Bucket range
        sigma: Standard deviation for probability calculation
        weights: Dict with weights for each source, e.g., {"ecmwf": 0.5, "hrrr": 0.3, "metar": 0.2}
    
    Returns:
        Weighted probability based on ensemble forecast
    """
    # Default weights if not provided
    if weights is None:
        weights = {"ecmwf": 0.5, "hrrr": 0.3, "metar": 0.2}
    
    # Collect available forecasts and their weights
    forecasts = []
    total_weight = 0.0
    
    if ecmwf_temp is not None:
        forecasts.append((float(ecmwf_temp), weights["ecmwf"]))
        total_weight += weights["ecmwf"]
    
    if hrrr_temp is not None:
        forecasts.append((float(hrrr_temp), weights["hrrr"]))
        total_weight += weights["hrrr"]
    
    if metar_temp is not None:
        forecasts.append((float(metar_temp), weights["metar"]))
        total_weight += weights["metar"]
    
    # If no forecasts available, return 0
    if not forecasts:
        return 0.0
    
    # Normalize weights
    if total_weight > 0:
        forecasts = [(temp, w/total_weight) for temp, w in forecasts]
    
    # Calculate weighted probability
    weighted_prob = 0.0
    for temp, weight in forecasts:
        prob = bucket_prob(temp, t_low, t_high, sigma)
        weighted_prob += prob * weight
    
    return round(weighted_prob, 4)


def get_ensemble_forecast(snapshots, city_slug, date):
    """
    Get ensemble forecast from multiple sources for a specific city and date.
    
    Args:
        snapshots: Dict of forecast snapshots
        city_slug: City identifier
        date: Date string (YYYY-MM-DD)
    
    Returns:
        Dict with ensemble forecast and individual source forecasts
    """
    ...
    if ecmwf is not None:
        forecasts.append(float(ecmwf))
        weights.append(0.5)  # ECMWF weight
    
    if hrrr is not None:
        forecasts.append(float(hrrr))
        weights.append(0.3)  # HRRR weight
    
    if metar is not None:
        forecasts.append(float(metar))
        weights.append(0.2)  # METAR weight
    ...
    if forecasts:
        ...
        if total_weight > 0:
            ...
            ensemble_temp = sum(f * w for f, w in zip(forecasts, normalized_weights))
        else:
            ensemble_temp = sum(forecasts) / len(forecasts)
    else:
        ensemble_temp = None
    
    return {
        "ensemble": ensemble_temp,
        "ecmwf": ecmwf,
        "hrrr": hrrr,
        "metar": metar,
...
def should_exit_for_forecast_change(forecast_temp, bucket_low, bucket_high, unit):
    ...
    if bucket_low == -999:
        return forecast_temp > float(bucket_high) + buffer
    if bucket_high == 999:
        return forecast_temp < float(bucket_low) - buffer

    mid_bucket = (bucket_low + bucket_high) / 2
    forecast_far = abs(forecast_temp - mid_bucket) > (abs(mid_bucket - bucket_low) + buffer)
    return not in_bucket(forecast_temp, bucket_low, bucket_high) and forecast_far
...
def save_market(market):
    ...
    write_json_atomic(p, market)

def market_priority(market, file_path: Path):
...
def load_all_markets():
    ...
    for f in MARKETS_DIR.glob("*.json"):
        try:
            ...
            pass
            continue
        key = (market.get("city", ""), market.get("date", ""))
        priority = market_priority(market, f)
...
def batch_fetch_forecasts(all_dates):
    """Fetch forecasts for ALL cities concurrently using ThreadPoolExecutor.
    This is 5-10x faster than sequential fetching."""
    results = {}
    
    def fetch_city(city_slug):
        try:
            return city_slug, take_forecast_snapshot(city_slug, all_dates)
        except Exception as e:
            print(f"  [WARN] {city_slug} fetch error: {e}")
            return city_slug, {}
    
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(fetch_city, slug): slug for slug in LOCATIONS.keys()}
        for future in as_completed(futures):
            city_slug, snapshot = future.result()
            results[city_slug] = snapshot
...
def scan_and_update():
    ...
    # Batch fetch all forecasts concurrently (5-10x faster)
    dates = [(now + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(4)]
    all_forecasts = batch_fetch_forecasts(dates)
    ...
    for city_slug, loc in LOCATIONS.items():
        ...
        try:
            dates = [(now + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(4)]
            snapshots = all_forecasts.get(city_slug) or {}
            if not snapshots:
        ...
        for i, date in enumerate(dates):
            ...
            # ================================================================
            # EXIT LOGIC — price-based
            # ================================================================
            # Step 1: Update trailing stop
            # Step 2: Take profit (price >= target → sell for sure profit)
            # Step 3: Stop loss (price drops 30% from entry → cut)
            # Step 4: Trailing stop hit → lock profit
            # ================================================================
            ...
            if mkt.get("position") and mkt["position"].get("status") == "open":
                ...
                if current_price is not None:
                    ...
                    if current_price >= entry * 1.20 or trailing_active:
                        remaining = 1.0 - current_price
                        new_stop = round(current_price - (remaining * 0.20), 4)
                    ...
                    # --- STEP 2: TAKE PROFIT ---
                    if current_price >= min(entry * 2.0, TAKE_PROFIT_MAX):
                        ...
                        pos["status"]       = "closed"
                        closed += 1
                        print(f"  [TP] {loc['name']} {date} | entry ${entry:.3f} → ${current_price:.3f} | PnL: +{pnl:.2f}")
                        save_market(mkt)
                    ...
                    # --- STEP 3: STOP LOSS ---
                        ...
                        pos["status"]       = "closed"
                        closed += 1
                        print(f"  [SL] {loc['name']} {date} | entry ${entry:.3f} → ${current_price:.3f} | PnL: {'+'if pnl>=0 else ''}{pnl:.2f}")
                        save_market(mkt)

                    # --- STEP 4: TRAILING STOP ---
                    elif trailing_active and pos.get("stop_price") and current_price <= pos["stop_price"]:
                        pnl = round((current_price - entry) * pos["shares"], 2)
                        balance += pos["cost"] + pnl
                        pos["closed_at"]    = snap.get("ts")
                        ...
                        pos["status"]       = "closed"
                        closed += 1
                        print(f"  [TRAIL] {loc['name']} {date} | entry ${entry:.3f} → ${current_price:.3f} | PnL: {'+'if pnl>=0 else ''}{pnl:.2f}")
                        save_market(mkt)
            ...
            if not mkt.get("position") and forecast_temp is not None and hours >= MIN_HOURS:
                ...
                if matched_bucket:
                    ...
                    if volume >= MIN_VOLUME:
                        # Use ensemble probability instead of single source
                        ensemble_forecast = get_ensemble_forecast(snapshots, city_slug, date)
                        p = ensemble_probability(
                            ensemble_forecast["ecmwf"], 
                            ensemble_forecast["hrrr"], 
                            ensemble_forecast["metar"], 
                            t_low, t_high, sigma
                        )
                        ev = calc_ev(p, ask)
                        if ev >= MIN_EV:
...
def print_status():
    print("\n" + render_status_text(build_report(DATA_DIR)) + "\n")

def print_report():
    report = build_report(DATA_DIR)
    closed_trades = [trade for trade in report["trades"] if trade["status"] != "active"]

    print(f"\n{'='*55}")
    print("  WEATHERBET — FULL REPORT")
    print(f"{'='*55}")
    print(f"  Total trades:    {report['total_trades']}")
    print(f"  Closed trades:   {report['closed_trades']}")
    print(f"  Wins/Losses:     {report['wins']} / {report['losses']}  (WR {report['win_rate']:.1f}%)")
    print(f"  Trail exits:     {report['trail_count']}")
    ...
    for trade in sorted(closed_trades, key=lambda item: (item['date'], item['city'])):
        reason = trade.get("close_reason") or trade.get("resolved_outcome") or trade["status"]
        print(
            f"    {trade['city']:<16} {trade['date']} | {trade['bucket']:<14} | "
            f"{trade['status']:<5} | PnL {trade['pnl']:+.2f} | {reason}"
        )
...
def monitor_positions():
    ...
    for mkt in open_pos:
        ...
        unit = mkt.get("unit", LOCATIONS.get(mkt["city"], {}).get("unit", "F"))
        ...
        forecast_temp = None
        try:
            snapshots = take_forecast_snapshot(mkt["city"], [mkt["date"]])
            forecast_temp = (snapshots.get(mkt["date"], {}) or {}).get("best")
        except Exception:
            snaps = mkt.get("forecast_snapshots", [])
            if snaps:
                forecast_temp = snaps[-1].get("best")

        # Trailing stop: once up 20%+, trail based on remaining spread
        # Spread-based: stop gets tighter as price approaches $1.00
        trailing_active = pos.get("trailing_activated", False)
        if current_price >= entry * 1.20 or trailing_active:
            remaining = 1.0 - current_price
            new_stop = round(current_price - (remaining * 0.20), 4)
            ...
            if new_stop > old_stop:
                ...
                pos["trailing_activated"] = True
                trailing_active = True
                print(f"  [TRAIL] {city_name} {mkt['date']} — stop ${new_stop:.3f} (price ${current_price:.3f}, remaining ${remaining:.2f})")
        ...
        if current_price >= min(entry * 2.0, TAKE_PROFIT_MAX):
            ...
            pos["status"]       = "closed"
            closed += 1
            print(f"  [TP] {city_name} {mkt['date']} | entry ${entry:.3f} → ${current_price:.3f} | PnL: +{pnl:.2f}")
            save_market(mkt)
            continue
        ...
        if current_price <= entry * (1 - STOP_LOSS_PCT):
            ...
            pos["status"]       = "closed"
            closed += 1
            print(f"  [SL] {city_name} {mkt['date']} | entry ${entry:.3f} → ${current_price:.3f} | PnL: {'+'if pnl>=0 else ''}{pnl:.2f}")
            save_market(mkt)
            continue

        # --- TRAILING STOP ---
        exit_triggered = trailing_active and pos.get("stop_price") and current_price <= pos["stop_price"]
        if exit_triggered:
            pnl = round((current_price - entry) * pos["shares"], 2)
            balance += pos["cost"] + pnl
            pos["closed_at"]    = datetime.now(timezone.utc).isoformat()
            ...
            pos["status"]       = "closed"
            closed += 1
            print(f"  [TRAIL] {city_name} {mkt['date']} | entry ${entry:.3f} → ${current_price:.3f} | PnL: {'+'if pnl>=0 else ''}{pnl:.2f}")
            save_market(mkt)
...
def run_loop():
    ...
    print(f"  Balance:    ${BALANCE:,.0f} | Max bet: ${MAX_BET:.2f}")
    print(f"  Exit:       TP ${TAKE_PROFIT_MAX:.2f} | SL -{STOP_LOSS_PCT:.0%} | Trail spread-based")
    ...
    while True:
        ...
        if now_ts - last_full_scan >= SCAN_INTERVAL:
            ...
            try:
                ...
                # Run integrations after scan
                if resolved > 0:
                    # Run self-learning
                    try:
                        print("  🧠 Running self-learning...")
                        subprocess.run([sys.executable, "self_learn.py"], 
...
if __name__ == "__main__":
    ...
    if cmd == "run":
        try:
            lock_handle = acquire_lock(RUN_LOCK_FILE)
